...
const emptyPlan: Omit<EmailPlan, 'id' | 'position'> = {
  name: '',
  description: '',
  badge: '',
  price_monthly: null,
...
export default function EmailPlansPage() {
  ...
  const [form, setForm] = useState<Omit<EmailPlan, 'id' | 'position'>>(emptyPlan)
  const [featuresText, setFeaturesText] = useState('')
  const [saving, setSaving] = useState(false)

  const load = async () => {
    setLoading(true)
    const { data } = await supabase.from('site_email_plans').select('*').order('position')
    setPlans(data ?? [])
    setLoading(false)
  }
  ...
  const save = async () => {
    ...
    if (editing) {
      await supabase.from('site_email_plans').update(payload).eq('id', editing.id)
    } else {
      const maxPos = plans.reduce((m, p) => Math.max(m, p.position), -1)
      await supabase.from('site_email_plans').insert({ ...payload, position: maxPos + 1 })
    }
  ...
  const remove = async (id: string) => {
    ...
    await supabase.from('site_email_plans').delete().eq('id', id)
    load()
  }

  const move = async (p: EmailPlan, dir: -1 | 1) => {
    const idx = plans.findIndex(x => x.id === p.id)
    const swapIdx = idx + dir
    if (swapIdx < 0 || swapIdx >= plans.length) return
    const swap = plans[swapIdx]
    await supabase.from('site_email_plans').update({ position: swap.position }).eq('id', p.id)
    await supabase.from('site_email_plans').update({ position: p.position }).eq('id', swap.id)
    load()
  }
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-black text-gray-900">Planos de Email Corporativo</h1>
            <p className="text-gray-500 text-sm">Gerir planos e preços de email corporativo</p>
          </div>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-2xl overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center text-gray-400">Nenhum plano de email configurado.</div>
        ) : (
          ...
          <div className="bg-white rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                ...
                <input className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#F5B700]" value={form.badge ?? ''} onChange={e => setForm(f => ({ ...f, badge: e.target.value }))} placeholder="MAIS POPULAR, EMPRESARIAL..." />
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-gray-600 text-sm font-medium block mb-1">Preço Mensal (AOA)</label>
                ...
                <div>
                  <label className="text-gray-600 text-sm font-medium block mb-1">Preço Anual (AOA)</label>
              ...
              <div>
                ...
                <textarea className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-[#F5B700] resize-none" rows={5} value={featuresText} onChange={e => setFeaturesText(e.target.value)} />
              </div>
