...
function AdminSidebar({ onClose }: { onClose?: () => void }) {
  ...
  return (
    <aside className="flex flex-col h-full w-64 flex-shrink-0" style={{ background: sidebarBg, borderRight: sidebarBdr }}>
      {/* Logo */}
      <div className="px-5 py-4 flex items-center justify-between" style={{ borderBottom: sidebarBdr }}>
        <Link href="/" aria-label="ViralizaHost" className="flex flex-col gap-1">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[10px] font-semibold" style={{ color: '#F5B700' }}>Painel Administrativo</span>
          </div>
        </Link>
        {onClose && (
          <button onClick={onClose} className="ml-2 p-1.5 rounded-lg md:hidden" style={{ color: 'rgba(255,255,255,0.4)' }} aria-label="Fechar menu">
            <X size={18} />
          </button>
        )}
      </div>
      ...
      <div className="px-5 pt-5 pb-1.5">
        <span className="text-[10px] font-black tracking-widest uppercase" style={{ color: 'rgba(255,255,255,0.20)' }}>Administração</span>
      </div>
      ...
      {/* Footer */}
      <div className="px-3 pb-5 space-y-0.5" style={{ borderTop: sidebarBdr, paddingTop: '1rem' }}>
