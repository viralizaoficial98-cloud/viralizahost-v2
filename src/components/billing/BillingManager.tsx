...
import {
  ...
  Search, Filter, ChevronLeft, ExternalLink,
} from 'lucide-react'
...
const SERVICE_TYPE_LABELS: Record<string, string> = {
  hosting: 'Hospedagem', email: 'Email', domain: 'Domínio',
  vps: 'VPS', dedicated: 'Dedicado', reseller: 'Revenda', other: 'Serviço',
}
...
function fmtCurrency(amount: number, currency: string) {
  ...
  if (currency === 'AKZ') return `${n.toLocaleString('pt-AO', { minimumFractionDigits: 2 })} Kz`
...
function isOverdue(due: string | null) { return !!due && new Date(due) < new Date() }

function getServiceLabel(inv: Invoice) {
  if (inv.order_items?.length > 0) {
    return inv.order_items.map(oi =>
      `${oi.service_name}${oi.quantity > 1 ? ` ×${oi.quantity}` : ''}`
    ).join(', ')
  }
  if (Array.isArray(inv.items) && inv.items.length > 0) {
    return inv.items.map((it: any) => it.name ?? it.description ?? 'Serviço').join(', ')
  }
  return '—'
}

function getServiceType(inv: Invoice) {
  if (inv.order_items?.length > 0) {
    const types = [...new Set(inv.order_items.map(oi => SERVICE_TYPE_LABELS[oi.service_type] ?? oi.service_type))]
    return types.join(' · ')
  }
  return '—'
}
        ...
        <div key={s} className="flex items-center gap-1 flex-1">
          <div className="flex items-center gap-1.5">
            <div className="w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold transition-all"
              style={i <= idx ? { background: 'linear-gradient(135deg,#F5B700,#D9A300)', color: '#0B0B0D' } : { background: '#F1F5F9', color: '#94A3B8' }}>
              {i < idx ? <Check size={11} /> : i + 1}
            </div>
            <span className="text-xs font-semibold hidden sm:block" style={{ color: i <= idx ? '#0B0B0D' : '#94A3B8' }}>{labels[i]}</span>
          </div>
          {i < steps.length - 1 && <div className="flex-1 h-0.5 mx-1 rounded" style={{ background: i < idx ? '#F5B700' : '#F1F5F9' }} />}
        </div>
      ))}
    </div>
  )
...
export default function BillingManager({ currency }: Props) {
  // KPIs
  ...
  const [payableInvs,  setPayable]      = useState<Invoice[]>([])
  ...
  const loadKpis = useCallback(async () => {
    setKpisLoading(true)
    try {
      const res  = await fetch('/api/client/billing/summary', { cache: 'no-store', credentials: 'include' })
  ...
  const loadInvoices = useCallback(async (p: number, s: string, st: string) => {
    setListLoading(true)
    try {
      const params = new URLSearchParams({ page: String(p), per_page: String(PER_PAGE) })
      if (s)  params.set('search', s)
      if (st) params.set('status', st)
      const res  = await fetch(`/api/client/billing/invoices?${params}`, { cache: 'no-store', credentials: 'include' })
      const data = await res.json()
      if (res.ok) {
  ...
  useEffect(() => { loadInvoices(page, search, statusFilter) }, [loadInvoices, page, search, statusFilter])
  ...
  const handleSearch = (v: string) => {
    ...
    searchRef.current = setTimeout(() => loadInvoices(1, v, statusFilter), 400)
  }

  // ── Open payment modal ─────────────────────────────────────────────────────
  const openPayModal = async (preselect?: Invoice) => {
    setLoadingPay(true)
    setStep(preselect ? 'review' : 'select')
    setSelected(preselect ?? null)
    setMethod(''); setPaymentId(''); setFile(null)
    setShowModal(true)
    try {
      ...
      setPayable(list)
      if (!preselect && list.length === 1) { setSelected(list[0]); setStep('review') }
  ...
  const createIntent = async () => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) {
        if (data.existing_payment_id) { showToast('error', `Já existe um pagamento ${data.existing_status} para esta fatura.`); return }
        throw new Error(data.error ?? 'Erro ao criar pagamento.')
      }
      setPaymentId(data.payment.id)
      if (['bank_transfer', 'multicaixa'].includes(method)) setStep('proof')
      else showToast('error', 'Pagamento automático não configurado ainda.')
  ...
  const uploadProof = async () => {
    ...
    try {
      ...
      setStep('done')
      await Promise.all([loadKpis(), loadInvoices(page, search, statusFilter)])
    } catch (e: unknown) { showToast('error', e instanceof Error ? e.message : 'Erro ao enviar comprovativo.') }
    finally { setUploading(false) }
  }

  const downloadPdf = async (inv: Invoice) => {
    try {
      const res = await fetch(`/api/client/billing/invoices/${inv.id}/pdf`, { credentials: 'include' })
      if (!res.ok) { showToast('error', 'Erro ao gerar PDF.'); return }
      const blob = await res.blob()
      const url  = URL.createObjectURL(blob)
      const a    = document.createElement('a')
      a.href     = url; a.download = `factura-${inv.invoice_number}.pdf`; a.click()
      URL.revokeObjectURL(url)
    } catch { showToast('error', 'Erro ao gerar PDF.') }
  }

  const openProof = async (inv: Invoice) => {
    try {
  ...
  const fmt = (n: number, cur?: string) => fmtCurrency(n, cur ?? currency)
  const totalPages = Math.ceil(totalCount / PER_PAGE)

  const kpiCards = kpis
    ? [
        { label: 'Total pago',  value: fmt(kpis.total_paid),         accent: '#059669', bg: 'rgba(16,185,129,0.06)', border: 'rgba(16,185,129,0.15)', Icon: TrendingUp },
        { label: 'Pendente',    value: fmt(kpis.total_pending),       accent: '#F59E0B', bg: 'rgba(245,159,11,0.06)', border: 'rgba(245,159,11,0.15)', Icon: Clock },
        { label: 'Em análise',  value: fmt(kpis.total_under_review),  accent: '#2563EB', bg: 'rgba(59,130,246,0.06)', border: 'rgba(59,130,246,0.15)', Icon: Eye },
        {
          label: kpis.next_due ? 'Próxima cobrança' : (kpis.credit > 0 ? 'Crédito' : 'Crédito'),
          value: kpis.next_due ? fmtDate(kpis.next_due) : fmt(kpis.credit),
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {kpiCards.map((card, i) => (
          <motion.div key={i}
            ...
            style={{ background: card?.bg ?? '#F8FAFC', border: `1px solid ${card?.border ?? '#E5E7EB'}` }}>
            {kpisLoading || !card ? (
              ...
              <>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-xs font-semibold" style={{ color: card.accent }}>{card.label}</span>
                </div>
                <div className="text-xl font-black" style={{ color: '#0B0B0D' }}>{card.value}</div>
              </>
            )}
          </motion.div>
        ))}
      ...
      <div className="rounded-2xl overflow-hidden"
        ...
        <div className="px-6 py-4" style={{ borderBottom: '1px solid #F1F5F9' }}>
          <div className="flex items-center gap-3 mb-4">
            ...
            <button onClick={() => loadInvoices(page, search, statusFilter)}
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <div className="flex items-center gap-2 px-3 py-2 rounded-xl flex-1 min-w-48"
              ...
              <input
                ...
                placeholder="Pesquisar por número, domínio…"
                className="flex-1 text-sm bg-transparent outline-none"
                style={{ color: '#374151' }}
              />
              {search && (
                <button onClick={() => handleSearch('')}><X size={12} style={{ color: '#94A3B8' }} /></button>
              )}
            </div>
        ...
        {listLoading ? (
          <div className="py-12 text-center">
            <Loader2 size={24} className="animate-spin mx-auto" style={{ color: '#F5B700' }} />
            <p className="text-sm mt-3" style={{ color: '#94A3B8' }}>A carregar faturas…</p>
          </div>
        ) : invoices.length === 0 ? (
          <div className="py-16 text-center">
            ...
            <p className="font-semibold text-sm mb-1" style={{ color: '#0B0B0D' }}>
              {search || statusFilter ? 'Nenhuma fatura encontrada' : 'Nenhuma fatura ainda'}
            </p>
            <p className="text-xs" style={{ color: '#94A3B8' }}>
              {search || statusFilter ? 'Tente alterar os filtros' : 'As suas faturas aparecerão aqui após a primeira subscrição'}
            </p>
          </div>
        ) : (
            ...
            <table className="w-full">
              <thead>
                <tr style={{ borderBottom: '1px solid #F1F5F9', background: '#F8FAFC' }}>
                  {['Nº Fatura', 'Serviço', 'Tipo', 'Domínio', 'Valor', 'Em aberto', 'Estado', 'Data', 'Ações'].map(h => (
                ...
                {invoices.map((inv, i) => {
                  ...
                  return (
                    <tr key={inv.id}
                      ...
                      {/* Serviço */}
                      <td className="px-4 py-4">
                        <div className="text-xs font-semibold max-w-[160px] truncate" style={{ color: '#374151' }}>
                          {getServiceLabel(inv)}
                        </div>
                      </td>
                      ...
                      {/* Tipo */}
                      <td className="px-4 py-4">
                        <div className="text-xs max-w-[100px] truncate" style={{ color: '#94A3B8' }}>
                          {getServiceType(inv)}
                        </div>
                      </td>
                      ...
                      {/* Domínio */}
                      <td className="px-4 py-4">
                        <div className="text-xs font-mono max-w-[120px] truncate" style={{ color: '#6B7280' }}>
                          {inv.domain_name ?? (inv.notes?.replace(/^Domínio:\s*/i, '') ?? '—')}
                        </div>
                      </td>

                      {/* Valor */}
                      <td className="px-4 py-4 text-sm font-bold whitespace-nowrap" style={{ color: '#0B0B0D' }}>
                      ...
                      {/* Data */}
                      <td className="px-4 py-4 text-xs whitespace-nowrap" style={{ color: over ? '#DC2626' : '#94A3B8' }}>
                        {over && <AlertTriangle size={11} className="inline mr-1" style={{ verticalAlign: 'middle' }} />}
                        {fmtDate(inv.due_date ?? inv.created_at)}
                      </td>
                      ...
                      <td className="px-4 py-4">
                        <div className="flex items-center gap-1.5">
                          ...
                          <Link href={`/billing/invoices/${inv.id}`}
                            ...
                            style={{ background: 'rgba(245,183,0,0.08)', border: '1px solid rgba(245,183,0,0.20)' }}>
                            <ExternalLink size={13} style={{ color: '#D9A300' }} />
                          </Link>

                          {/* Pagar */}
                          {payable && (
                            <button onClick={() => openPayModal(inv)}
                              className="text-xs font-bold px-2.5 py-1.5 rounded-lg text-black transition-all hover:opacity-80"
                          ...
                          {/* Ver comprovativo */}
                          {inv.proof_file && (
                            <button onClick={() => openProof(inv)} title="Ver Comprovativo"
                              className="p-1.5 rounded-lg transition-colors hover:opacity-70"
              ...
              <div className="px-6 py-5 overflow-y-auto flex-1">
                ...
                {step === 'select' && (
                  <div className="space-y-3">
                    {loadingPay ? (
                    ...
                    ) : payableInvs.length === 0 ? (
                      <div className="py-10 text-center">
                        <div className="w-14 h-14 rounded-2xl mx-auto mb-3 flex items-center justify-center"
                          style={{ background: 'rgba(16,185,129,0.08)', border: '1px solid rgba(16,185,129,0.15)' }}>
                          <CheckCircle2 size={26} style={{ color: '#059669' }} />
                        </div>
                        <p className="font-semibold text-sm mb-1" style={{ color: '#0B0B0D' }}>Não existem faturas pendentes.</p>
                        <p className="text-xs mb-5" style={{ color: '#94A3B8' }}>Todas as suas faturas estão em dia.</p>
                      ...
                      <>
                        <p className="text-sm font-semibold mb-3" style={{ color: '#64748B' }}>Selecione a fatura que deseja pagar:</p>
                        {payableInvs.map(inv => {
                          const s    = STATUS_MAP[inv.status] ?? STATUS_MAP.pending
                  ...
                  <div className="space-y-4">
                    ...
                    {PAYMENT_METHODS.map(m => (
                      <button key={m.id} onClick={() => setMethod(m.id)}
                        className="w-full text-left rounded-xl p-4 flex items-center gap-4 transition-all"
                        style={{ background: method === m.id ? 'rgba(245,183,0,0.06)' : '#F8FAFC', border: `2px solid ${method === m.id ? '#F5B700' : '#F1F5F9'}` }}>
                        <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0"
                    ...
                    <button onClick={createIntent} disabled={!method || creating}
                ...
                {step === 'proof' && selected && (
                  <div className="space-y-4">
                    {method === 'bank_transfer' && (
                      <div className="rounded-xl p-4 space-y-3" style={{ background: 'rgba(245,183,0,0.04)', border: '1px solid rgba(245,183,0,0.15)' }}>
                        ...
                        {[
                          ...
                          { label: 'Valor exacto',      value: fmtCurrency(selected.outstanding, selected.currency) },
                    ...
                    <p className="text-xs font-semibold uppercase tracking-wider" style={{ color: '#94A3B8' }}>Dados da transferência</p>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input value={transferRef} onChange={e => setTransferRef(e.target.value)}
                          className="w-full border rounded-xl px-3 py-2 text-sm outline-none" style={{ borderColor: '#E5E7EB' }} placeholder="Nº doc / ref…" />
                      </div>
                      <div>
                        <label className="text-xs font-bold block mb-1" style={{ color: '#64748B' }}>Data da transferência</label>
                        <input type="date" value={transferDate} onChange={e => setTransferDate(e.target.value)}
                          className="w-full border rounded-xl px-3 py-2 text-sm outline-none" style={{ borderColor: '#E5E7EB' }} />
                      </div>
                    </div>
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input value={payerName} onChange={e => setPayerName(e.target.value)}
                          className="w-full border rounded-xl px-3 py-2 text-sm outline-none" style={{ borderColor: '#E5E7EB' }} placeholder="Nome…" />
                      </div>
                      <div>
                        ...
                        <input value={payerBank} onChange={e => setPayerBank(e.target.value)}
                          className="w-full border rounded-xl px-3 py-2 text-sm outline-none" style={{ borderColor: '#E5E7EB' }} placeholder="Banco…" />
                      </div>
                    </div>
                    <div>
                      <label className="text-xs font-bold block mb-1" style={{ color: '#64748B' }}>Valor transferido</label>
                      <input value={declaredAmt} onChange={e => setDeclaredAmt(e.target.value)} type="number" step="0.01"
                        className="w-full border rounded-xl px-3 py-2 text-sm outline-none" style={{ borderColor: '#E5E7EB' }}
                    ...
                    <div>
                      ...
                      <button onClick={() => fileInputRef.current?.click()}
                        ...
                        style={{ background: file ? 'rgba(16,185,129,0.04)' : '#F8FAFC', border: `2px dashed ${file ? '#6EE7B7' : '#E5E7EB'}` }}>
                        {file
                    ...
                    <button onClick={uploadProof} disabled={!file || uploading}
                      className="w-full py-3 rounded-xl font-bold text-black flex items-center justify-center gap-2 disabled:opacity-50 transition-all hover:opacity-90"
                      style={{ background: 'linear-gradient(135deg,#F5B700,#D9A300)', boxShadow: '0 4px 14px rgba(245,183,0,0.30)' }}>
                ...
                {step === 'done' && (
                  <div className="py-8 text-center">
                    ...
                    <p className="text-sm mb-1" style={{ color: '#64748B' }}>O pagamento encontra-se em análise.</p>
                    <p className="text-xs mb-6" style={{ color: '#94A3B8' }}>A nossa equipa irá validar o pagamento e actualizar o estado da fatura em breve.</p>
